		...
		unsigned int offset)
{
	...
	__le32 *addr_array;
	raw_node = F2FS_NODE(node_page);//page->f2fs_node
	addr_array = blkaddr_in_node(raw_node);//page is inode, or data
	return le32_to_cpu(addr_array[offset]);///pointer to block_address
}
#ifdef penta1
static inline block_t datablock_addr(struct page *node_page,
		unsigned int offset)
 ...
 */
#ifdef penta
int f2fs_lba_r(u64 maxbytes,struct bio * bio ,struct inode *inode);
int f2fs_lba_rh(u64 maxbytes,struct inode *inode, struct list_head *lba_list);
